...
first_section="$(jq -r '[.blocks[] | select(.type == "heading" and .level == 2) | .i] | first // 9999' "$blocks")"
lead_in="$(jq -r --argjson fs "$first_section" \
...
if [[ "$lead_in" -eq 0 ]]; then
  while IFS=$'\t' read -r start end; do
    if sed -n "${start},${end}p" "$src_hint" 2>/dev/null \
       | grep -qiE '(contents|on this page|in this (document|page)|tl;dr|sommaire)'; then
      prose_toc=1
      break
    fi
  done < <(jq -r --argjson fs "$first_section" \
