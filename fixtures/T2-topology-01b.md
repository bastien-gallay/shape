...
healthy. An arrow points from a service to what it needs.

```mermaid
graph TD
  web[web-console] --> edge[api-edge]
  edge --> auth[auth-gateway]
  edge --> queue[queue-broker]
  auth --> cache[token-cache]
  queue --> store[object-store]
  ledger[ledger-writer] --> queue
  ledger --> store
  report[report-builder] --> auth
  report --> store
```
