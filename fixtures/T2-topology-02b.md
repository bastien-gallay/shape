...
Dependencies are not transitive in the table below — read the rows you
need, then read their rows.

| Service | Calls |
| --- | --- |
| `dock-portal` | `gateway-api` |
| `agent-console` | `gateway-api`, `manifest-api` |
| `gateway-api` | `auth-broker`, `booking-api`, `tariff-api` |
| `auth-broker` | `identity-store`, `session-cache` |
| `identity-store` | `pg-primary` |
| `session-cache` | — |
| `booking-api` | `slot-planner`, `event-bus`, `pg-primary` |
| `slot-planner` | `berth-registry`, `event-bus` |
| `berth-registry` | `pg-primary` |
| `tariff-api` | `rate-engine`, `object-vault` |
| `rate-engine` | `pg-primary` |
| `manifest-api` | `event-bus`, `object-vault`, `customs-relay` |
| `customs-relay` | `event-bus` |
| `event-bus` | — |
| `ledger-writer` | `event-bus`, `pg-primary`, `object-vault` |
| `invoice-render` | `ledger-writer`, `object-vault` |
| `notify-fanout` | `event-bus`, `template-store` |
| `template-store` | `object-vault` |
| `object-vault` | — |
| `pg-primary` | — |
